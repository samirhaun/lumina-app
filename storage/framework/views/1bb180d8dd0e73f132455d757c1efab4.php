<?php $__env->startSection('title', 'Gerenciar Linktree'); ?>
<?php $__env->startSection('header', 'Gerenciador de Links'); ?>
    ...
    <script>
        $(function() {
            ...
            $('#linktreeSettingsForm').submit(e => {
                ...
                $.post("<?php echo e(route('admin.linktree-manager.settings.update')); ?>", $(e.target).serialize())
                    .done(res => Swal.fire({
                        icon: 'success',
                        title: 'Salvo',
                        text: res.success,
            ...
            const table = $('#linksTable').DataTable({
                ...
                processing: true,
                serverSide: false,
                ajax: "<?php echo e(route('admin.linktree-manager.data')); ?>",
                columns: [{
                        data: 'display_order',
                        orderable: false,
      ...
      `,
            ...
            function makeSortable() {
                $('#linksTable tbody').sortable({
                    ...
                    update() {
                        ...
                        $.post("<?php echo e(route('admin.linktree-manager.update-order')); ?>", {
                                order
                            })
            ...
            $('#linksTable').on('click', '.delete-btn', function() {
                ...
                Swal.fire({
                    ...
                    $.ajax({
                            url: `<?php echo e(url('admin/linktree-manager')); ?>/${id}`,
                            type: 'DELETE'
                        })
                        .done(r => {
            ...
            $('#linkForm').submit(e => {
                ...
                const url = id ?
                    `<?php echo e(url('admin/linktree-manager')); ?>/${id}` :
                    `<?php echo e(route('admin.linktree-manager.store')); ?>`;
            ...
            const socialTable = $('#socialsTable').DataTable({
                ...
                processing: true,
                serverSide: false,
                ajax: "<?php echo e(route('admin.linktree-manager.socials.data')); ?>",
                columns: [{
                        data: 'display_order',
                        orderable: false,
                        render: () => ' <i class="fas fa-arrows-alt-v"></i>'
         ...
         <button class="btn btn-danger btn-sm del-social"  data-id="${r.id}"><i class="fas fa-trash"></i></button>`
            ...
            function makeSocialSortable() {
                $('#socialsTable tbody').sortable({
                    ...
                    update() {
                        ...
                        $.post("<?php echo e(route('admin.linktree-manager.socials.order')); ?>", {
                                order
                            })
            ...
            $('#socialForm').submit(e => {
                ...
                const url = id ?
                    `<?php echo e(url('admin/linktree-manager/socials')); ?>/${id}` :
                    `<?php echo e(route('admin.linktree-manager.socials.store')); ?>`;
                const type = id ? 'PUT' : 'POST';
                const btn = $('button[type=submit]', e.target),
                    html = btn.html();
                btn.prop('disabled', true).html('<span class="spinner-border spinner-border-sm"></span>');
            ...
            $('#socialsTable').on('click', '.del-social', function() {
                ...
                Swal.fire({
                    ...
                    if (!res.isConfirmed) return;
                    $.ajax({
                            url: `<?php echo e(url('admin/linktree-manager/socials')); ?>/${id}`,
                            type: 'DELETE'
                        })
                        .done(r => {
                            Swal.fire('Removido!', r.success, 'success');
...
<?php echo $__env->make('admin::layouts.layout', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\Users\Bruno\Documents\lumina-app\lumina-app\Modules/Admin\resources/views/linktree/index.blade.php ENDPATH**/ ?>