...
<?php $__env->startSection('title', 'Movimentações Financeiras'); ?>
...
<?php $__env->startPush('scripts'); ?>
    ...
    <script>
        $(function() {
            ...
            let categories = <?php echo json_encode($categories, 15, 512) ?>; // Torna a variável mutável
            const dtLanguage = {
            ...
            const transactionsTable = $('#transactionsTable').DataTable({
                processing: true,
                ajax: "<?php echo e(route('admin.financial-transactions.data')); ?>",
                columns: [{
            ...
            $('[data-bs-target="#transactionModal"]').on('click', function() {
                ...
                $('#transactionForm').data('method', 'POST').data('url',
                    '<?php echo e(route('admin.financial-transactions.store')); ?>');
            ...
            $('#transactionsTable tbody').on('click', '.delete-btn', function() {
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        $.ajax({
                            url: `<?php echo e(url('admin/financial-transactions')); ?>/${data.id}`,
                            type: 'DELETE',
                            success: function(r) {
            ...
            $('#categoryForm').on('submit', function(e) {
                e.preventDefault();
                $.post("<?php echo e(route('admin.financial-categories.store')); ?>", $(this).serialize()).done(
                    response => {
                        Swal.fire({
                            icon: 'success',
            ...
            $('#categoryList').on('click', '.delete-category-btn', function() {
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        $.ajax({
                            url: `<?php echo e(url('admin/financial-categories')); ?>/${data.id}`,
                            type: 'DELETE',
                            success: response => {
                                Swal.fire({
                                    icon: 'success',
...
<?php echo $__env->make('admin::layouts.layout', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\Users\Samir\Desktop\lumina-app\Modules/Admin\resources/views/financial-transactions/index.blade.php ENDPATH**/ ?>